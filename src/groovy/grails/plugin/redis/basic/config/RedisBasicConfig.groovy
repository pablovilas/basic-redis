package grails.plugin.redis.basic.config
    ...
    static def configureService = {redisConfigMap, key, serviceClass ->
        ...
        if (sentinels && masterName) {
            ...
            if (sentinels instanceof Collection) {
                "redis${key}Pool"(JedisSentinelPool, masterName, sentinels as Set, ref(poolBean), timeout, password, database) { bean ->
            ...
            "redis${key}Pool"(JedisPool, ref(poolBean), host, port, timeout, password, database) { bean ->
                bean.destroyMethod = 'destroy'
            }
        }

        "redis${key}Service"(serviceClass) {
            redisPool = ref("redis${key}Pool")
        }
    }
